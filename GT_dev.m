...
% for cyklus pro všechny obrázky ze skupiny velidační dat
for i = 1:1000
    % přeuložení názvu do tabulky GT spolu s cestou k novému obrázku
    nazev = char(dev{i,1}(1,1));
    GT{i,1} = {char(strcat([cesta 'dev\224\'],nazev(1:end-4),'.png'))};
    
    % načtení původního obrázku
    img = imread(char(strcat([cesta 'dev\'],nazev)));
...
%     
    ...
    imwrite(new,strcat([cesta 'dev\224\'],nazev(1:end-4),'.png'));
    
    % vypočtení zmenšení obrazu
    scale = [224 224]./size(img,[1 2]);
    
    % nalezení počtu objektů v obraze
    num_object = sum(~cellfun(@isempty,dev(i,2:end)));
...
obr = imread([cesta 'data\dev\224\08010.jpg']);
RGB = insertShape(obr,'Rectangle',GT{10,2}{1,1},'Color', 'green','Opacity',0.7,'LineWidth',5);
imshow(RGB);

% uložení matice anotovaných dat pro trénovací obrazy
save([cesta 'dev\224\GTdev.mat'],'GT');