...
GT = table('Size',[1000,2],'VariableTypes',{'cell','cell'});

% for cyklus pro všechny obrázky ze skupiny testovacích dat
for i = 1:1000
    % přeuložení názvu do tabulky GT spolu s cestou k novému obrázku
    nazev = char(testgt{i,1}(1,1));
    nazev = nazev(6:end);
    GT{i,1} = {char(strcat([cesta 'test\224\'],nazev(1:end-4),'.png'))};
    
    % načtení původního obrázku
    img = imread(char(strcat([cesta 'test\'],nazev)));
    ...
    imwrite(new,strcat('C:\Users\Matous\Documents\DP\test\224\',nazev(1:end-4),'.png'));
    
    % vypočtení zmenšení obrazu
    scale = [224 224]./size(img,[1 2]);
    
    % nalezení počtu objektů v obraze
    num_object = sum(~cellfun(@isempty,testgt(i,2:end)));
...
obr = imread([cesta 'data\test\224\09004.jpg']);
RGB = insertShape(obr,'Rectangle',GT{4,2}{1,1},'Color', 'green','Opacity',0.7,'LineWidth',5);
imshow(RGB);

% uložení matice anotovaných dat pro trénovací obrazy
save('C:\Users\Matous\Documents\DP\test\224\GTtest.mat','GT');